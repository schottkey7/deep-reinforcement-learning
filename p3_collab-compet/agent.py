...
class Agent():
    ...
    def __init__(self, state_size, action_size, num_agents, random_seed):
        ...
        if os.path.exists('saved/actor_checkpoint.pth'):
            save = torch.load('saved/actor_checkpoint.pth', map_location='cpu')
            self.actor_local.load_state_dict(save)

        if os.path.exists('critic_checkpoint.pth'):
            save = torch.load('saved/critic_checkpoint.pth', map_location='cpu')
            self.critic_local.load_state_dict(save)
...
class OUNoise:
    ...
    def sample(self):
        ...
        x = self.state
        dx = self.theta * (
            self.mu - x) + self.sigma * np.random.standard_normal(self.size)
        self.state = x + dx
        return self.state
